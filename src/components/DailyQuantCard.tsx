// ==========================
// File: src/components/DailyQuantCard.tsx
// ==========================
...
export default function DailyQuantCard({ apiBase, getHeaders, onOpen }: {
  ...
  if (loading) return <div className="text-xs text-white/70">Loading daily quant pick…</div>;
  ...
  const Item = ({it, label}:{it:any, label:string}) => it ? (
    <div className="p-3 rounded-xl border border-white/10 bg-white/5">
      ...
      <div className="text-xs text-white/70">P(up) {Math.round((it.prob_up_end||0)*100)}% · median {Math.round(it.median_return_pct||0)}% · {it.horizon_days}d</div>
      <div className="mt-1 text-sm text-white/80">{it.blurb}</div>
